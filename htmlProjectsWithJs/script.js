...
if (FoundNumbers[0] != null) {
    document.getElementById('output').innerHTML = `</br><hr style='width:50%'>Output: ${FoundNumbers}`.fontcolor('cyan').bold().fixed();
} else {
    document.getElementById('output').innerHTML = `</br><hr style='width:50%'>Cannot take in ${UserInput}. Too large`.fontcolor('cyan').bold().fixed();
}
